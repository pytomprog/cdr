...
MappedFrameBuffer::MappedFrameBuffer(const FrameBuffer *buffer, MapFlags flags)
{
	if (buffer->planes().empty()) {
		std::cerr << "Buffer hasn't planes !!!" << std::endl;
		return;
	}
	...
	for (const FrameBuffer::Plane &plane : buffer->planes()) {
		...
		if (plane.offset > length ||
			...
			std::cerr << "plane is out of buffer: buffer length=" << length<< ", plane offset=" << plane.offset << ", plane length=" << plane.length << std::endl;
			return;
		}
	...
	for (const FrameBuffer::Plane &plane : buffer->planes()) {
		...
		if (!info.address) {
			...
			if (address == MAP_FAILED) {
				error_ = -errno;
				std::cerr << "Failed to mmap plane: " << std::strerror(-error_) << std::endl;
				return;
			}
